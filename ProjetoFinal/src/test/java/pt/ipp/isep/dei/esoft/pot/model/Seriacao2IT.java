...
/**
 * Teste para a Seriação 2.
 *
 * @author Jordão e António
 */
public class Seriacao2IT {

    /**
     * Teste do método Seriar da classe Seriação 2.
     */
    @Test
    public void testSeriar() {
        System.out.println("Seriar2");
        lerFicheiros();
        Anuncio anun = AplicacaoPOT.getInstance().getPlataforma().getRegistoAnuncios().getAnuncioById("12345");
        List<Candidatura> auxiliar = new ArrayList<>(AplicacaoPOT.getInstance().getPlataforma().getRegistoAnuncios().getAnuncioById("12345").getM_oListaCandidaturas().getCandidaturas());
        Candidatura[] expResult = new Candidatura[5];
        expResult[0] = auxiliar.get(4);
        expResult[1] = auxiliar.get(3);
        expResult[2] = auxiliar.get(2);
        expResult[3] = auxiliar.get(0);
        expResult[4] = auxiliar.get(1);
